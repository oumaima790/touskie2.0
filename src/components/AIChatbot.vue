<template>
  <div>
      ...
      <div v-if="isOpen" class="chat-panel">
        <div class="chat-header">
          <div class="header-left">
            ...
            <div>
              ...
              <div class="header-sub">{{ isTyping ? 'Thinking...' : 'Online · Ready to help' }}</div>
            </div>
          </div>
        ...
        <div class="chat-input-row">
          <input
            ...
            :disabled="isTyping"
          />
          <button class="send-btn" @click="send" :disabled="!input.trim() || isTyping">
    ...
    <button class="fab" @click="isOpen = !isOpen">
      <transition name="icon-swap" mode="out-in">
        <v-icon v-if="!isOpen" key="open" color="white" size="24">mdi-robot-excited</v-icon>
        <v-icon v-else key="close" color="white" size="22">mdi-close</v-icon>
      </transition>
...
.fab:hover { transform: scale(1.08); box-shadow: 0 6px 28px rgba(249, 115, 22, 0.6); }
.fab-badge {
  position: absolute;
  top: -4px;
  right: -4px;
...
.chip:hover { background: #f97316; color: white; }
.chat-input-row {
  display: flex;
  align-items: center;
  gap: 8px;
...
.chat-input:focus { border-color: #f97316; background: white; }
...
.chat-bar-enter-active, .chat-bar-leave-active { transition: all 0.3s cubic-bezier(0.34, 1.56, 0.64, 1); }
.chat-bar-enter-from, .chat-bar-leave-to { opacity: 0; transform: translateY(20px) scale(0.95); }
.icon-swap-enter-active, .icon-swap-leave-active { transition: all 0.15s; }
.icon-swap-enter-from { transform: rotate(-90deg); opacity: 0; }
.icon-swap-leave-to { transform: rotate(90deg); opacity: 0; }
</style>